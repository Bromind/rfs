...
impl RfsServer {
    pub fn new(name: String, config: RfsConfig) -> Option<Self> {
        ...
        match my_conf {
            Ok(&Field::Server{ref name, key: _, ref address, ref port}) => {
                welcome(my_conf.unwrap().clone());
                let socket = address.clone() + ":" + &port;
                match TcpListener::bind(socket) {
                    Ok(l) => Some(RfsServer {name: name.clone(), config: config.clone(), listener: l}),
                    Err(e) => {error!("Can not create RfsServer. Reason: {}", e); None},
                }
            },
            Ok(&Field::Client{ref name, key: _}) => {
                error!("Item {} is a client", name); None
            },
            Err(e) => {
    ...
    fn get_identity(&self, s: String) -> Option<Client> {
        match self.get_config_key(s.clone()) {
            ...
            Err(e) => {warn!("Can not get client identity. Reason: {}", e); None},
        }
    }
}
